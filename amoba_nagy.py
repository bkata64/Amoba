...
def nyert(palya, jatekos, sor, oszlop):
...
def lef(palya, jatekos, sor, oszlop):
    ...
    col = oszlop + 1
    row = sor + 1
    while col < len(palya) and row < len(palya) and palya[row][col] == jatekos["szimbolum"]:
        szamol += 1
        col += 1
        row += 1
    return szamol == 5
...
def felf(palya, jatekos, sor, oszlop):
    ...
    col = oszlop + 1
    row = sor - 1
    while col < len(palya) and row >= 0 and palya[row][col] == jatekos["szimbolum"]:
        szamol += 1
        col += 1
        row -= 1
    col = oszlop - 1
    row = sor + 1
    while col >= 0 and row < len(palya) and palya[row][col] == jatekos["szimbolum"]:
        szamol += 1
        col -= 1
        row += 1
    return szamol == 5
...
print(f"A megadott értékek, csak '1' és '{n}' között érvényesek sorban és oszlopban is")
...
if gyoztes: 
    ...
    print("Vége a játéknak, az eredmény döntetlen.")
